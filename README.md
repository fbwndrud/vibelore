...
- 원작의 인물·상태를 그대로 가져와 각색하고, 방향을 확인한 뒤 장면을 대사까지 한 장으로 생성하는 웹툰 제작 흐름이 따라옵니다.
...
```text
각색 방향 ─▶ 장면별 영어 연출 ─▶ 생성 전 검증 ─▶ 대사 포함 장면 이미지 ─▶ 실제 시각 검토
  (승인)         (자동)          hard 차단      호스트가 생성           완성본
```
...
- **웹툰 각색.** 원작 상태를 가져와 각색 방향·화풍·칸 수를 확인한 뒤 장면을 대사까지 한 장의 세로형 이미지로 완성합니다.
...
1. **방향 정하기.** 그림체, 말풍선·글자 표현, 세로 스크롤 여부, 칸 수를 묻습니다.
2. **기준 그림.** 인물과 장소의 기준 그림을 먼저 그려 확인받습니다. 이후 모든 장면이 이 그림을 따릅니다.
3. **장면 각색.** 원작 범위를 정하고 영어 연출 지시를 만들어 생성 전 검증을 거칩니다.
4. **완성.** 장면 전체를 대사까지 한 장의 세로형 이미지로 그리고 실제 그림을 검토합니다. 위 제작 예시들이 이 방식입니다.
...
쓸 수 있습니다. 집필 언어는 작품 프로필에서 정하고, 인터뷰는 사용자가 쓰는 언어로 진행합니다. 안내 문서는 한국어 원문과 영어판(`*.en.md`)이 있습니다. 위 [작품 언어](#작품-언어)를 보세요.
</details>

## 더 읽기

- [시작 안내](docs/GETTING_STARTED.md) — 등록, 첫 작품, 다음 화, 막혔을 때
- [웹툰 제작](docs/WEBTOON.md) — 각색 방향, 필수 선택, 장면 통합 제작과 승인
