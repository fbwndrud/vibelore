...
사용자가 집필 언어를 밝히면 BCP 47 태그로 정규화해 `lore_profile`의 선택 인자 `language`로 넘긴다 — 일본어 → `ja`, 브라질 포르투갈어 → `pt-BR`, 번체 중국어 → `zh-Hant`. 밝히지 않았으면 인자를 생략해 이미 정해진 언어를 그대로 쓰고, 언어를 확인하는 질문을 새로 만들지 않는다. 서로 다른 언어를 두 번 말했을 때만 어느 쪽인지 묻는다.
