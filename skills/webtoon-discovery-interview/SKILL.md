...
description: Vibelore 소설·세계관·캐릭터를 웹툰으로 각색할 때 기존 설정을 상속하고 웹툰 표현의 차이만 인터뷰한다. 방향·각색·기준 이미지·시각 샘플·최종 승인과 Codex 이미지 생성·수정 연결을 진행한다. 다음 회차는 저장된 결정과 승인 참조에서 이어간다.
---

# 웹툰 발견 인터뷰

`lore_webtoon_plan`으로 시작한다. 원작 프로젝트의 `project`와 `workId`를 사용하고, 읽은 원작 범위만 `sourceChapters`로 지정한다. 이미 승인된 소설 의도는 읽어서 재사용하며 웹툰의 새 취향으로 재해석하지 않는다.

새 회차는 `segmented=true`로 시작해 공통 회차 방향을 유지하면서 장면별 각색·부분 검토·전체 흐름 검토를 진행한다. 기존 workflow를 이어갈 때는 저장된 분할 정책을 유지한다.

사용자가 러프·컷별 작화 대신 **장면 전체를 대사까지 한 장으로** 만들기를 명시적으로 고르면 `lore_webtoon_scene`으로 바꾼다. 이미 확정한 이미지 API 선택과 참조 이미지를 재사용하고, `needs_interview`의 칸 수 질문(`4, 6, 8, 9, auto`)은 사용자에게 그대로 묻는다. `auto`는 각색마다 AI가 3~12칸 중 고르고, 3칸 미만 선택은 응답 `warnings`의 연속성 경고를 사용자에게 보여준다. 세부는 [장면 통합 제작](../../docs/reference/WEBTOON_WORKFLOW.md#장면-통합-제작--명시적-선택-경로)을 따른다.
