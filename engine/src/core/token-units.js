 ...
 */
import { PROMPT_FAMILY_KO, PROMPT_FAMILY_MULTILINGUAL } from './language-policy.js';
...
/**
 * Pick the budget estimator for a prompt family.
 *
 * The ko family (and a call that names no family — the implicit legacy ko
 * path) keeps the caller's own pre-0.4.0 estimator so ko prompts stay
 * byte-identical: tokenUnits() charges the ASCII/markdown/JSON that fills
 * real ko prompts at the Latin rate and would admit different material.
 * Every other family measures with the script-aware tokenUnits().
 */
export function budgetEstimator(promptFamily, legacyEstimate) {
  if (promptFamily === undefined || promptFamily === null || promptFamily === PROMPT_FAMILY_KO) return legacyEstimate;
  if (promptFamily === PROMPT_FAMILY_MULTILINGUAL) return tokenUnits;
  throw new TypeError(`budgetEstimator: unknown prompt family '${String(promptFamily)}'`);
}
