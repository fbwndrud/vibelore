 ...
 */
export function buildColdOpenInstruction() {
    const lines = ['', '## 1화 독자 계약', ''];
    lines.push(`첫 ${COLD_OPEN_WINDOW_CHARS}자 안에서 설정을 설명하기보다 다음 기능을 장면으로 수행한다:`,
    ...
    for (const a of STRONG_EVENT_CATALOG) {
        lines.push(`  - ${a.label}: ${a.hint}`);
    }
    lines.push('', '위 유형보다 작품의 StoryIdentity와 인물 선택이 우선이다. 자극적 사건을 억지로 삽입하지 않는다.');
    return lines.join('\n');
}
