...
사용자 질문과 승인은 사용자가 처리합니다. 소설 집필은 서로 의존하지 않는 요청(상태 추출,
프로필 검사, 검토들)을 한 번의 `needs_model`에 묶어 보내므로 호스트는 이를 병렬로 답할 수
있고, 추출 결과가 필요한 의미 연속성 검사는 그다음 왕복에 옵니다. 앱이나 서버가 꺼진 동안 작업이 저절로 계속되는
