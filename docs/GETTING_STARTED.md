...
> 1화를 웹툰으로 만들어 줘. 그림체와 글자 표현부터 물어보고, 장면을 대사까지 한 장으로 그려 줘.

그림은 OpenAI 이미지 API(별도 키·과금)를 호스트가 호출해 그리고, 인물·배경 참조 이미지 파일이 필요합니다.
원작 범위·칸 수·이미지 모델과 비용은 먼저 확인받고, 소설과 웹툰은 따로 저장합니다. 완성본은 장면마다
대사까지 들어간 PNG/JPEG 이미지와 `scene.html`이며 `.vibelore/webtoon/candidates/` 아래에 남습니다.
