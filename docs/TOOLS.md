...
검사 뒤 계획·계약이 바뀌었거나 손수정을 `lore_sync`로 발행했다면, 인자 없는 `lore_write`가 같은
원고를 현재 계약으로 다시 검사합니다(새 초고 없음). 새 `instruction`을 줄 때만 초고를 새로 씁니다.
...
소설은 현재 또는 지정 워크플로의 상세와 검사 영수증, 보존·승인 대기 중인 원고
(`draftProse`)를 읽습니다. 모델 응답은 노출하지 않습니다. 웹툰은 `lane="webtoon"`과 `detail`로 상세를 선택하며 full에 원작·계획이 포함될 수 있습니다.
