/* 길 위의 번개 — 장면 웹툰 뷰어. data.json 하나로 렌더링. 의존성 없음. */
(function () {
  'use strict';
  ...
  function findingsBlock(s) {
    const head = el('div', { class: 'fhead' + (s.verdict === 'pass' ? ' pass' : '') },
      ...
      el('span', null, T('자동 재생성 없이 첫 결과와 판정을 그대로 공개합니다. 검토는 오케스트레이션 호스트의 자기검토입니다.')));
    const ul = el('ul');
    for (const f of s.findings) {
      ul.append(el('li', null,
        el('span', { class: 'fl ' + (f.severity === 'blocking' ? 'b' : 'a') }, T(f.severity === 'blocking' ? '차단' : '참고')),
        el('span', { lang: EN ? 'en' : 'ko' }, fx(f)),
