...
(function () {
  ...
  const num = (n) => (window.I18N ? I18N.num(Math.round(n)) : Math.round(n).toLocaleString('ko-KR'));
  ...
  const dur = (s) => s == null ? '–' : s < 60 ? T('{n}초', { n: Math.round(s) }) : s < 3600 ? T('{m}분 {s}초', { m: Math.floor(s / 60), s: Math.round(s % 60) }) : T('{h}시간 {m}분', { h: Math.floor(s / 3600), m: Math.round((s % 3600) / 60) });
  const KIND = { real: ['실측', 'k-real'], list: ['정가 환산', 'k-list'], est: ['추정', 'k-est'], none: ['기록 없음', 'k-none'] };
  const kind = (k) => el('span', { class: 'kind ' + KIND[k][1] }, T(KIND[k][0]));
  const ch = (c) => T('{n}화', { n: c });
  ...
  function epTable() {
    ...
    for (const e of C.episodes) {
      ...
      t.append(el('tr', null,
        el('td', null, el('b', null, T('{n}화', { n: ch }))),
        el('td', { class: 'l' }, hostName(e, 'novel')),
        el('td', null, nv ? tk(tot(nv)) : '–', nv ? null : el('span', { class: 'sub' }, T('기록 없음'))),
        el('td', null, usd(nvCost.v), ' ', kind(nvCost.k)),
        el('td', null, nvSec != null ? dur(nvSec) : wall(ch) ? el('span', null, dur(wall(ch)), el('span', { class: 'sub' }, T('워크플로 경과'))) : '–'),
        el('td', { class: 'l' }, e.webtoonPending ? el('span', { class: 'muted' }, T('제작 중')) : hostName(e, 'webtoon')),
        ...
        el('td', null, el('b', null, sum > 0 ? usd(sum) : '–'), partial ? el('span', { class: 'sub' }, T(e.webtoonPending ? '소설만' : '일부 미집계')) : null)));
    }
  }
  ...
  function tokChart() {
    ...
    for (const e of C.episodes) for (const [lane, lb] of [['novel', '소설'], ['webtoon', '웹툰 각색']]) { const r = row(e.chapter, lane); if (r && r.tokKnown) rows.push([`${ch(e.chapter)} ${T(lb)}`, lane === 'novel' ? e.novelModel : e.model, r]); }
    const max = Math.max(...rows.map((x) => tot(x[2])));
    const chart = el('div', { class: 'tchart', role: 'img', 'aria-label': T('회차별 토큰 구성 막대. 아래 표에 같은 숫자가 있습니다.') });
    ...
    for (const [lb, model, r] of rows) {
      ...
      for (const [k, name, cls] of SEG) if (r[k] > 0) {
        ...
        const show = (ev) => { tip.textContent = ''; tip.append(el('b', null, lb), el('br'), T('{name} {n} 토큰 ({pct}%)', { name: T(name), n: num(r[k]), pct: Math.round((r[k] / tot(r)) * 100) }), el('br'), T('호출 {n}회 · 합계 {sum}', { n: r.tokKnown, sum: num(tot(r)) })); tip.classList.remove('hidden'); tip.style.left = Math.min(ev.clientX + 12, innerWidth - tip.offsetWidth - 8) + 'px'; tip.style.top = (ev.clientY + 14) + 'px'; };
        s.addEventListener('mousemove', show); s.addEventListener('mouseleave', () => tip.classList.add('hidden'));
        bar.append(s);
      }
      chart.append(el('div', { class: 'trow' }, el('div', { class: 'tl2' }, lb, el('small', null, model)), el('div', null, bar), el('em', null, tk(tot(r)))));
    }
    box.append(chart, el('div', { class: 'tlegend' }, SEG.map(([, name, cls]) => el('span', null, el('i', { class: cls }), T(name)))),
      el('p', { class: 'note', style: 'margin-top:10px' }, T('Claude·Codex·OpenAI API 호출은 이번 제작에서 요청마다 캐시를 새로 만들어 대부분 캐시 쓰기로 잡혔습니다. Grok은 캐시 없이 새 입력으로 읽습니다. 같은 일을 해도 공급자 방식에 따라 막대 모양이 달라집니다.')));
  }
  ...
  function stepTabs() {
    ...
    for (const c of chs) box.append(el('button', { class: 'eptab' + (c === st.ch ? ' active' : ''), onclick: () => { st.ch = c; stepTable(); } }, el('span', null, st.lane === 'arc' ? T('{n}화 전', { n: c }) : ch(c))));
  ...
  function stepTable() {
    ...
    t.append(el('tr', null, ['단계', '호출', '새 입력', '캐시 쓰기', '캐시 읽기', '출력', '(그중 추론)', '시간', '비용'].map((h) => el('th', null, T(h)))));
    ...
    const line = (r, cls) => { const c = costOf(r); return el('tr', { class: cls }, el('td', { class: 'l' }, T(r.label), r.tokKnown < r.calls ? el('span', { class: 'sub' }, T('{n}회 토큰 기록 없음', { n: r.calls - r.tokKnown })) : null),
    ...
    const e = ep(st.ch);
    $('#step-note').textContent = st.lane === 'novel' ? T('{ch}화 소설 · {host}. 한 단계가 여러 번 호출되면(예: 수정 뒤 재검토) 합산했습니다.', { ch: st.ch, host: hostName(e, 'novel') }) + (st.ch === 3 ? T(' 3화는 호출별 소요 시간이 남지 않아 시간 칸이 비어 있습니다.') : '')
      : st.lane === 'webtoon' ? T('{ch}화 웹툰 각색 · {host}. 장면 {n}개의 계획·검증 호출 합계입니다. 그림 검토는 오케스트레이터가 직접 해서 토큰 기록이 없습니다.', { ch: st.ch, host: hostName(e, 'webtoon'), n: e.sceneCount })
        : T('{ch}화를 쓰기 전에 다음 아크를 설계한 호출입니다(아크 설계와 설계 검증).', { ch: st.ch });
  }

  /* ---------- 단계별 평균 ---------- */
  function avgTable() {
    const t = $('#avg-table');
    t.append(el('tr', null, ['단계', '기록된 회차', '호출', '호출당 입력', '호출당 출력', '호출당 시간', '호출당 비용'].map((h) => el('th', null, T(h)))));
    ...
    for (const s of steps) {
      ...
      t.append(el('tr', { id: `step-${s}` }, el('td', { class: 'l' }, T(rs[0].label), el('span', { class: 'sub' }, s)),
        el('td', null, chs.map((c) => ch(c)).join(' ')), el('td', null, a.calls),
  ...
  function imgTable() {
    ...
    for (const e of C.episodes) {
      ...
      t.append(el('tr', null, el('td', null, ch(e.chapter)), el('td', null, im.length), el('td', null, usd(u), ' ', kind('real')), el('td', null, usd(u / im.length)), el('td', null, dur(s / im.length))));
    }
    t.append(el('tr', null, el('td', { class: 'l', colspan: 5 }, el('span', { class: 'muted small' }, T('모델 {m}, 1024×1536, 품질 high. 비용은 OpenAI usage API 집계로, 차이는 참조 이미지 수(4~8장)에서 옵니다.', { m: C.imageModel })))));
  }

  /* ---------- 계산기 ---------- */
  function calc() {
    const novelOpts = C.episodes.filter((e) => row(e.chapter, 'novel')).map((e) => ({ ch: e.chapter, label: `${e.novelModel} (${T('{n}화 실측', { n: e.chapter })}${e.chapter <= 3 ? T(', 구버전') : ''})`, r: row(e.chapter, 'novel') }));
    const toonOpts = C.episodes.filter((e) => row(e.chapter, 'webtoon')).map((e) => ({ ch: e.chapter, label: `${e.model} (${T('{n}화 실측', { n: e.chapter })}${e.chapter === 1 ? T(', 정가 환산') : ''})`, r: row(e.chapter, 'webtoon'), scenes: e.sceneCount }));
    ...
    const form = el('div', { class: 'cform' },
      el('label', null, T('몇 화를 쓸까요'), el('input', { type: 'number', id: 'c-n', min: 1, max: 300, value: 10 })),
      el('label', null, T('소설 모델'), sel('c-novel', novelOpts, pickDefault(novelOpts)), el('small', null, T('그 모델로 쓴 회차 하나의 실측을 그대로 곱합니다.'))),
      el('label', { class: 'chk' }, el('input', { type: 'checkbox', id: 'c-toon', checked: 'checked' }), T('웹툰도 만들기')),
      el('div', { class: 'row' },
        el('label', null, T('각색 모델'), sel('c-adapt', toonOpts, pickDefault(toonOpts))),
        el('label', null, T('화당 장면 수'), el('input', { type: 'number', id: 'c-scenes', min: 1, max: 20, value: 7 }), el('small', null, T('이 작품은 7~9장면')))));
    ...
    const update = () => {
      ...
      out.innerHTML = '';
      const perX = (total, each, unit) => T('{total} ({unit} {each})', { total, each, unit: T(unit) });
      const parts = [el('div', { class: 'small muted' }, T('{n}화 예상 비용', { n })), el('div', { class: 'big2' }, usd(perEp * n), el('small', null, T('화당 {v}', { v: usd(perEp) }))),
        el('dl', null,
          el('dt', null, T('소설 쓰기')), el('dd', null, nvUsd == null ? T('금액 기록 없음') : perX(usd(nvUsd * n), usd(nvUsd), '화당')),
          toon ? [el('dt', null, T('웹툰 각색 (장면 {n}개)', { n: sc })), el('dd', null, perX(usd(adPer * sc * n), usd(adPer), '장면당')),
            el('dt', null, T('이미지')), el('dd', null, perX(usd(imgUsd * sc * n), usd(imgUsd), '장당'))] : null,
          el('dt', null, T('모델 작업 시간')), el('dd', null, perX(dur(secEp * n), dur(secEp), '화당')),
          el('dt', null, T('답변 모델 토큰')), el('dd', null, perX(tk(tokEp * n), tk(tokEp), '화당'))),
        nvUsd == null ? el('div', { class: 'warn' }, T('이 소설 모델은 금액이 기록되지 않아 합계에서 빠졌습니다. 토큰만 참고하세요.')) : null,
        nv.ch <= 3 ? el('div', { class: 'warn' }, T('구버전(0.3.0)으로 쓴 회차라 현재 버전보다 호출이 많습니다. 4화 이후 회차를 기준으로 고르는 편이 현재에 가깝습니다.')) : null,
        el('div', { class: 'note' }, T('작업을 지휘하는 호스트(Claude Code·Codex 세션)의 비용과 사람의 검토 시간은 포함하지 않았습니다. 시간은 호출을 순서대로 실행했을 때의 합이라, 장면을 병렬로 돌리면 줄어듭니다.'))];
  ...
  }).catch((err) => { $('#calc-body').append(el('p', { class: 'note' }, T('데이터를 불러오지 못했습니다: ') + err.message)); });
})();
