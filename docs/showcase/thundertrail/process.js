...
(function () {
  ...
  const REJECT_KO = {
    ...
    INVALID_SCENE_TEXT_ASSIGNMENT: '대사를 칸·순간에 배정한 형식이 규격에 맞지 않아 반려',
    INCOMPLETE_SCENE_IMAGE_REVIEW: '그림 검토 답이 모든 비트를 다루지 않아 반려(오케스트레이터 답변)',
  };
  ...
  function renderChanges() {
    ...
    const items = [
      ...
      { c: 5, s: '소설', t: '멈춰 있던 5화 워크플로를 Opus 5.5로 이어 씀', d: '5화 소설 워크플로는 전날 Gemini로 시도하다 API 한도·크레딧 문제로 첫 요청(화별 계획)에서 멈췄습니다. Gemini 답변은 하나도 반영되지 않았고, 다음 날 같은 워크플로를 lore_write로 다시 열어 모든 모델 답변을 claude-opus-5-5가 냈습니다. 타임라인은 멈춰 있던 시간을 빼고 잽니다.' },
      { c: 6, s: 's1', t: 'gpt-6-luna가 옮겨 적은 해시 오기를 하네스가 바로잡음', d: '사전 검증 답에 계획 해시를 그대로 옮겨 적어야 하는데 luna가 두 글자를 빠뜨려 서버가 “오래된 검증”으로 두 번 반려했습니다. 하네스가 스키마 값과 90% 이상 같은 해시 오기만 스키마 값으로 고치도록 바꾸고 모든 교정을 기록했습니다. 검증 내용은 건드리지 않았습니다.' },
      { c: 6, s: 's7', t: '6화 지시의 둔덕·뒤판·사슴 때문에 s7 검증이 세 번 막힘', d: '6화 공통 지시에 둔덕, 자루에 걸린 뒤판, 가지 뿔 사슴을 적어 두었는데 수레 앞 대화 장면(p63–71)에는 그런 장소·구조·등장물이 없어 검증이 막았습니다. 3·4·7화와 같은 유형입니다. 지시를 발췌 중립 문구로 바꾸고(인물 규칙은 유지) s7을 새 워크플로로 다시 시작했습니다. 새 s7 그림 검토에서는 오케스트레이터가 그림에 보인 비트만 적어 서버가 반려했고, 같은 그림의 검토 요청을 다시 받아 모든 비트를 적어 냈습니다(판정 내용은 같음).' },
      { c: 7, s: '아크', t: '아크 3 설계에 4화용 옛 방향 문구가 전달됨', d: '7~9화 아크를 gpt-6-sol이 자동 설계할 때, 하네스 패치 오류(오케스트레이터 실수)로 7~9화 줄기 대신 4화용 옛 방향 문구가 전달되었습니다. MCP 잘못이 아닙니다. 사용자와 상의해 결과를 그대로 두고 여기 공개하기로 했습니다.' },
      { c: 7, s: '소설', t: '7화 편집 점수가 8로 기록됨', d: '다른 항목은 100점 척도인데 편집 점수만 8입니다. 모델이 10점 척도로 답한 것으로 보이며 고치지 않고 그대로 둡니다.' },
      { c: 7, s: 's3', t: '둔덕·판자길 지시 때문에 마당 장면 검증이 세 번 막힘', d: '7화 공통 지시에 둔덕과 부러진 판자길을 적어 두었는데 마당 장면(p20–31)은 거기 가지 않아 검증이 막았습니다. 지시를 발췌 중립 문구로 바꾸고 s3을 새 워크플로로 다시 시작했습니다.' },
      { c: 7, s: 's5', t: '판자길 참조 이미지를 마지막 장면에만 붙임', d: '새 s3 그림의 마당 배경에 판자길 참조 이미지가 새어 들어왔습니다. s5부터 그 참조를 판자길에 실제로 가는 s8에만 붙였습니다. 이미 시작된 s4까지는 모든 참조를 붙인 채로 두었습니다.' },
