...
(function () {
  ...
  function renderHead() {
    ...
    $('#chip-image').textContent = `${DATA.imageHost || 'Codex · OpenAI API'} · ${DATA.imageModel}`;
  ...
  function findingsBlock(s) {
    ...
    const head = el('div', { class: 'fhead' + (s.verdict === 'pass' ? ' pass' : '') },
      ...
      el('span', null, regen
        ...
        : `자동 재생성 없이 첫 결과와 판정을 그대로 공개합니다. 검토는 ${(e0 => (e0.reviewer || DATA.reviewer).model)(ep())}(오케스트레이션 호스트)가 했으며 독립 평가가 아닙니다.`));
    const ul = el('ul');
    for (const f of s.findings) {
      ul.append(el('li', null,
        el('span', { class: 'fl ' + (f.severity === 'blocking' ? 'b' : 'a') }, f.severity === 'blocking' ? '차단' : '참고'),
        el('span', null, f.correction ? el('b', null, '[정정] ') : null, f.derived ? el('b', null, '[서버 판정] ') : null, f.ko),
