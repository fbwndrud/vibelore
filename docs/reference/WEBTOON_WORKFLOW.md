...
4. `asset:{path,inputHash,provenance}`로 반입한다. `webtoon-scene-image-review`는 실제 그림과 모든 문구·화자·읽기 순서를 확인한다. 관측한 글자를 기록하고 미열람 검토를 통과시키지 않는다. 우려가 남으면 `scene_needs_revision`으로 결과와 근거를 보여주며 자동 재생성하지 않는다.
