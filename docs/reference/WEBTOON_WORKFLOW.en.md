...
Existing per-panel work is not changed automatically. The approved API model choice and the character and background references are reused.
...
It adapts an existing Vibelore novel, world and characters into a vertical webtoon. It pins the source version and manages
...
`lore_webtoon_scene`. The output is vertical SVG/HTML, and the actual image generation is done by the host.
...
Setting `lore_webtoon_plan(segmented=true)` on a new job limits the size of tasks while keeping the existing tools and approval stages. The policy of existing jobs is not changed, and existing calls without `segmented` keep the earlier path. This option does not guarantee model capability or quality.
...
`lore_webtoon_plan.imageModel` sets the requested model candidate for a new job. If there is no stored choice, the default candidate is `gpt-image-2.5-sunburst`, and other explicit candidates are `gpt-image-2.5-flare` or the existing `gpt-image-2`. If the same work has a confirmed choice, it is reused. Existing stored policies and hashes are not changed automatically.
...
| `completed` | Use the local approved version and the SVG/HTML master. Start the next episode with `newWorkflow=true` |
...
Old workflows stored before this link, without `imagePolicy`, resume with the existing import method. To use reference image approval with the new link, finish or reject the existing job and start with `newWorkflow=true`. Stored approvals are not silently converted to the new contract.
