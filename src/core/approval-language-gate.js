...
function machineLeaf(key, path, value) {
  ...
  // ageBand is generated work-language text ("early twenties"); only the engine default is a sentinel.
  if (key === 'ageBand') return parent === 'intrinsic' && value === 'unknown';
...
export function approvalLanguageMessages({ artifact, workContract, epoch, attempt }) {
  const instruction = workContract.promptFamily === 'ko'
    ? '승인할 작품 산출물 전체의 생성된 자연어가 한국어인지 검토한다. 변경하거나 번역하지 말고 모든 제목·예시·설명·계획·설정 값을 읽는다. 기계 ID/enum 및 사용자 원문 출처는 언어 위반이 아니다. 근거로는 문장·구절을 담은 필드만 지목한다. id/…Id/to/status/unit/hash/버전/시각·enum 코드 값(male, alive, graphemes, strict 등)과 JSON 키는 지목하지 않는다. 고유명과 승인된 인용 예외만 허용하며, 정보가 불충분하면 uncertain이다.'
    : `Review ALL generated natural-language values in this approval artifact for the target language ${workContract.language}. Do not rewrite or translate. Include titles, examples, questions, explanations, plans and world/character text. Machine IDs/enums and actual user source fields are exempt. Cite as evidence only fields that hold generated sentences or phrases; never cite JSON keys, id/...Id/to/status/unit/hash/version/timestamp fields or enum code values such as male, alive, graphemes or strict. Only proper names and scoped approved quotation exceptions may use another language. Return uncertain if evidence is insufficient.`;
  return [
