...
한국어 | [English](HOSTS.en.md)

기억으로 쓰지 않고 확인한 것만 적습니다. 확인하지 못한 줄은 그렇게 표시했습니다.
포맷이 바뀌면 이 파일을 먼저 고치세요.

호스트 실호출 확인일: 2026-08-22<br>
서버 자체 handshake·기본 26개/고급 39개 도구 schema 확인일: 2026-09-04<br>
웹툰 통합 표면: 기본 30개/고급 43개(장면 경로 포함). 아래 과거 호스트 실호출 기록과 구분합니다.
...
당시 기본 26개와 고급 39개 도구 표면은 2026-09-04 로컬 MCP handshake와 자동 테스트로 확인했습니다. Codex의 비대화형 smoke는
...
실제 작화에는 이미지 생성·참조 이미지 첨부·로컬 파일 저장·이미지와 합성본 열람이
필요합니다. Codex 실행 계약은 [이미지 연결 안내](skills/webtoon-discovery-interview/references/codex-images.md)를
따릅니다. 다른 호스트는 동등한 기능을 실제로 제공하는지 확인한 뒤 요청을 실행합니다.
장면 경로는 사용자가 고른 OpenAI 이미지 API 모델만 호출하며 다른 모델로 자동 대체하지 않습니다. 컷별 경로(deprecated)는 선택한 모델을 지정할 수 없으면 `needs_image_runtime`에서 멈추며 유료 경로로 자동 대체하지 않습니다.

이 브랜치의 회귀 테스트는 합성 이미지로 장면 경로의 MCP 왕복·생성 전 검증·이미지 검토와, 컷별 경로(deprecated)의 러프 승인·최종 승인까지 검사합니다.
상단의 소설 호스트 실호출 기록이 웹툰의 라이브 이미지 생성·미감 검증을 뜻하지 않습니다.
완성 흐름과 제약은 [웹툰 안내](docs/WEBTOON.md)에 있습니다.
